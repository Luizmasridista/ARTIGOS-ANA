...
export function mapLoginError(err: unknown): { mensagem: string; retryAfter?: number } {
  if (err instanceof ApiError) {
    ...
    if (err.status === 401) return { mensagem: 'usuário inválido' }
...
export function validarLoginInput(nome: string): string | null {
  const t = nome.trim()
...
export function Home() {
  ...
  const handleLogin = async (nome: string) => {
    ...
    setErro(null)
    setRetryAfter(null)
    const err = validarLoginInput(nome)
    if (err) {
    ...
    <div className="home">
      <div style={{ width: '100%', maxWidth: 380, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="home-card" role="main" aria-labelledby="home-titulo">
          ...
          <p className="home-subtitulo">Escolha como entrar na biblioteca.</p>
